...
| `DEVSWITCH_PROXY_PROVIDER` | proxy プロバイダー (`native`) | `native`              | proxy start, info            |
...
# UI の host/port を指定（デフォルト: localhost:9001）
...
| `<tmpdir>/ui.pid`                | UI daemon PID                        |
| `<tmpdir>/ui.log`                | UI ログ（daemon 時のみ）             |
